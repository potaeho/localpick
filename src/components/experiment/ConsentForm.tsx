...
import { PRIVACY_CONTACT, PRIVACY_NOTICE } from "@/lib/consent";
import { getDevice } from "@/lib/events";

/**
 * 인터뷰 연락처 수집 — 설문과 분리된 별도 화면.
 *
      ...
      <div className="space-y-5 px-6 py-6">
        {!consentAvailable && (
          <p role="alert" className="rounded-lg bg-lp-gray-100 p-3 text-sm text-lp-gray-900">
            공개 문의 채널이 설정되기 전까지 인터뷰 연락처를 받을 수 없습니다.
          </p>
        )}
        <div>
          <label
        ...
        <label className="flex cursor-pointer items-start gap-3 text-sm text-lp-gray-900">
          ...
          />
          위 내용을 확인했고, 인터뷰 연락을 위한 개인정보 수집·이용에
          동의합니다.
        </label>
      </div>
