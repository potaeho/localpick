...
export default async function RegionPage({
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 lg:px-6 lg:py-8">
      ...
      <h1 className="mt-1 text-lp-heading text-lp-ink sm:text-lp-page-heading">
        {region.name}
      </h1>
      <p className="mt-lp-lg max-w-2xl text-sm leading-relaxed text-lp-gray-900 sm:text-base">
        {region.description}
      </p>
      ...
      {regionCreators.length > 0 && (
        <section className="mt-lp-section">
          ...
          <ul className="mt-lp-lg grid gap-lp-md sm:grid-cols-2">
            {regionCreators.map((creator) => (
              <li key={creator.id}>
                <Link
                  href={`/creators/${creator.id}`}
                  className="flex items-start gap-lp-lg rounded-lp-card border border-lp-gray-300 bg-white p-lp-lg hover:border-lp-green focus:outline-none focus-visible:ring-2 focus-visible:ring-lp-green focus-visible:ring-offset-2"
                >
                  <span
                    aria-hidden="true"
                    className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lp-circle bg-lp-green-light font-bold text-lp-green"
                  >
      ...
      <section className="mt-10">
        ...
        <ul className="mt-lp-xl grid grid-cols-2 gap-x-4 gap-y-8 md:grid-cols-3 lg:grid-cols-4">
